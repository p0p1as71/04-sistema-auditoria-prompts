...
1. **Modelo sugerido:** (Si no se indica uno específico, asigna siempre **GPT-4o**. Prohibido usar "agnóstico").
2. **Nivel:** (Principiante / Intermedio / Avanzado).
3. **Robustez:** (Alta / Media / Baja — Capacidad de resistir variaciones).
4. **Precisión:** (Usa siempre escala de estrellas: ⭐⭐⭐⭐⭐).
5. **Token-Efficient:** (SÍ / NO).
6. **JSON-Ready:** (SÍ / NO).
7. **Versión:** (Detecta si el prompt tiene versión propia; si no la indica explícitamente, asigna siempre **v1.0**).
